...
describe("/api/sightings/user/:user_id", () => {
  describe("GET", () => {
    test("200: Responds with sightings within the specified latitude/longitude range for a given user", () => {
      return request(app)
        .get(
          "/api/sightings/user/1?swlat=-10.0&swlong=30.0&nelat=10.0&nelong=50.0"
        )
        .expect(200)
        .then(({ body }) => {
    ...
    test("404: Returns No sightings found if no sightings are found within the specified latitude/longitude range", () => {
      return request(app)
        .get(
          "/api/sightings/user/1?swlat=-90.0&swlong=-180.0&nelat=-80.0&nelong=-170.0"
        )
        .expect(404)
        .then(({ body }) => {
          const { message } = body;
          expect(message).toBe(
            "No sightings found for the given user and coordinates"
          );
        });
    });
    test("400: Responds with a 400 error for invalid latitude/longitude values", () => {
      return request(app)
        .get(
          "/api/sightings/user/1?swlat=-100.0&swlong=30.0&nelat=10.0&nelong=50.0"
        )
        .expect(400)
        .then(({ body }) => {
  ...
  test("416: Responds with Range Error when the longitude and latitude values are invalid", () => {
    ...
    return request(app)
      ...
      .then(({ body }) => {
        const { message } = body;
        expect(message).toBe(
          "Range error of latitude (-90, +90) or longitude (-180, 180)"
        );
      });
  });
});
